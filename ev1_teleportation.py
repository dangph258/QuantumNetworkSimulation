...
def create_processor(depolar_rate, dephase_rate):
    ...
    # print("create process: ", depolar_rate, dephase_rate)
...
def write_resul(result_file, data, distance, comment = ""):
    aptr = open(result_file, "a")
    aptr.write(comment)
    #print(data[['depolar_rate' , 'fidelity']].iloc[1])
    for index, row in data.iterrows():
        aptr.write("%d\t%0.4f\n" % (distance, row['fidelity']))
        #aptr.write(f"{data[['depolar_rate']].iloc[index]}\t{data[['fidelity']].iloc[index]}\n")
    aptr.close()
...
def create_plot():
    ...
    num_runs=1000
    distance=10
    # depolar_rates = [100 * i for i in range(0, 11, 1)]
    # depolar_rates = [1,1e3,1e6]
    #print (depolar_rates)
    depolar_rate = 100
    dephase_rate = 100
    ...
    if (len(sys.argv) >= 3):
        depolar_rate = float(sys.argv[2])
    
    if (len(sys.argv) >= 4):
        dephase_rate = float(sys.argv[3])
    ...
    comment = "#Run experiment: num_runs: %s, distance: %s, depolar_rate: %s, dephase_rate: %s, result_file: %s \n" % (num_runs, distance, depolar_rate, dephase_rate, result_file)
    print(comment)
    fidelities = run_experiment(num_runs=num_runs, distance=distance,
    ...
    write_resul(result_file, data, distance, comment)
    #print(data)
